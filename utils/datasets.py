...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        #print("cache",cache)
        #print("[cache[x] for x in self.img_files]",[cache[x.replace("\\","/")] for x in self.img_files])
        #修改
        #labels, shapes = zip(*[cache[x.replace("/","\\")] for x in self.img_files])
        if "val" in self.img_files[0]:
